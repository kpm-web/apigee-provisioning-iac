...
locals {
  envgroups = { for key, value in var.apigee_envgroups : key => try(value.hostnames, tolist(value), []) }
  instances = { for key, value in var.apigee_instances : value.region => {
    name                  = key
    display_name          = try(value.display_name, null)
    description           = try(value.description, "Terraform-managed")
    ...
    access_logging        = try(value.access_logging, null)
    enable_nat            = try(value.enable_nat, false)
    activate_nat          = try(value.activate_nat, false)
  } }
}
...
module "kms-org-db" {
  source     = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/kms?ref=v58.0.0"
...
module "kms-inst-disk" {
  ...
  keyring = {
    ...
    name     = "${var.inst_kms_keyring_name_prefix}-${each.key}"
  }
...
module "apigee" {
  source     = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/apigee?ref=v58.0.0"
  project_id = var.project_id
  organization = {
